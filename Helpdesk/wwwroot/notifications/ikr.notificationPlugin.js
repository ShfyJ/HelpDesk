       ...
       */
        ...
        if ($.trim(parentId) != "" && parentId.length > 0) {
            $("#" + parentId).append("<div class='ikrNoti_Counter'></div>" +
                ...
                "</div>" +
                "<div class='ikrSeeAll'><a href='Manager/RequestManager'>See All</a></div>" +
                "</div>");
       ...
       */
        ...
        if ($.trim(parentId) != "" && parentId.length > 0) {
            ...
            $("#" + parentId + " .ikrNotifications .ikrSeeAll").click(function () {
                window.open('../' + defaultSettings.AreaName + '/' + defaultSettings.ControllerName +  '', '_blank');
            });
            
            var totalUnReadNoti = defaultSettings.NotificationList.filter(x => x.rStatus == "blue").length;
            ...
            if (defaultSettings.NotificationList.length > 0 ) {
                ...
                $.map(defaultSettings.NotificationList, function (item) {
                        ...
                        $("#" + parentId + " .ikrNotificationItems").append("<div class='ikrSingleNotiDiv" + className + "' notiId=" + item.notiId + ">" +
                           ...
                           // "<div class='ikrNotificationBody'>" + room + item[ikrLowerFirstLetter(defaultSettings.ListBodyPropName)] + "</div>" +
